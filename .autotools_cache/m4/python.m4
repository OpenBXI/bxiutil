...
# PC_PYTHON_CHECK_MODULE_REAL(LIBRARY, REAL, [ACTION-IF-FOUND], [ACTION-IF-NOT-FOUND])
# ----------------------------------------------------------------------
# Macro for checking if a Python library is installed
AC_DEFUN([PC_PYTHON_CHECK_MODULE_REAL],
[AC_REQUIRE([PC_PROG_PYTHON])[]dnl
m4_define([pc_python_safe_mod], m4_bpatsubsts($1, [\.], [_]))
m4_define([pc_python_safe_mod], m4_bpatsubsts($1, [-], [_]))
AC_CACHE_CHECK([for Python '$1' library],
...
AS_IF([test "$[pc_cv_python_module_]pc_python_safe_mod" = "no"], [$4], [$3])
])# PC_PYTHON_CHECK_MODULE_REAL
